...
func (tds *TemplateDataSources) Render(templates v1beta1.Templates) error {
	...
	for _, tmpl := range templates {
		...
		if tmpl.Secret {
			if existing, blocked := tds.SecretK8sObj.Data[tmpl.Name], isBlocked(blockedSecretData, tmpl.Name); blocked {
				// A non-template key (e.g. a native key generated by the operator)
				// already exists. Native credentials must never become template-owned;
				// connection aliases may be adopted during the dc10 templates migration.
				if tds.isNativeCredentialSecretKey(tmpl.Name) {
					return fmt.Errorf("%s already exists in the secret", tmpl.Name)
				}
				if !bytes.Equal(existing, tmplRes.Bytes()) && !tds.canAdoptBlockedSecretKey(tmpl.Name) {
					return fmt.Errorf("%s already exists in the secret", tmpl.Name)
				}
				// Value matches: treat it as if it were applied by this template so
				// cleanUpData keeps it and the annotation stays consistent.
			...
			if existing, blocked := tds.ConfigMapK8sObj.Data[tmpl.Name], isBlocked(blockedConfigMapData, tmpl.Name); blocked {
				if existing != tmplRes.String() {
...
func (tds *TemplateDataSources) isNativeCredentialSecretKey(key string) bool {
	switch tds.DatabaseK8sObj.Status.Engine {
	case consts.ENGINE_POSTGRES:
		return key == consts.POSTGRES_DB || key == consts.POSTGRES_USER || key == consts.POSTGRES_PASSWORD
	case consts.ENGINE_MYSQL:
		return key == consts.MYSQL_DB || key == consts.MYSQL_USER || key == consts.MYSQL_PASSWORD
	case consts.ENGINE_CLICKHOUSE:
		return key == consts.CLICKHOUSE_DB || key == consts.CLICKHOUSE_USER || key == consts.CLICKHOUSE_PASSWORD
	default:
		return false
	}
}

func (tds *TemplateDataSources) canAdoptBlockedSecretKey(key string) bool {
	if tds.DatabaseK8sObj.Status.Engine != consts.ENGINE_POSTGRES {
		return false
	}
	switch key {
	case "POSTGRES_HOST", "POSTGRES_PORT", "POSTGRES_SSLMODE", "POSTGRES_DSN", "DATABASE_URL":
		return true
	default:
		return false
	}

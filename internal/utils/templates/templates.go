	...
	for _, tmpl := range templates {
		...
		if tmpl.Secret {
			if existing, blocked := tds.SecretK8sObj.Data[tmpl.Name], isBlocked(blockedSecretData, tmpl.Name); blocked {
				...
				// already exists. Allow the template to take ownership only when it
				// produces the exact same value, making the declaration idempotent.
				if !bytes.Equal(existing, tmplRes.Bytes()) {
